...
function paginateit(content) {
    ...
    var show_per_page = 1; //change this to the amount you want to see for each set
    
    var number_of_items = $(content).children('.listData1').length;
    
    var number_of_pages = Math.ceil(number_of_items / show_per_page);
    
    if (number_of_items > show_per_page) {

        $(content).parent().find('.current_page1').val(0);
        $(content).parent().find('.show_per_page1').val(show_per_page);
        ...
        $(content).children('.listData1').css('display', 'none');      
        $(content).children('.listData1').slice(0, show_per_page).css('display', 'block');            
...
function load_navigation(page_num, content) {
    ...
    var show_per_page = $(content).parent().find('.show_per_page1').val();
    ...
    if (number_of_pages > max_pages_to_show) {
        var middle_items = 2;
        var current_page = $(content).parent().find('.current_page1').val();
    ...
    $(content).parent().find('#vpage_navigation').html(navigation_html);
...
function previous(content) {
    new_page = parseInt($(content).parent().find('.current_page1').val()) - 1;
...
function go_to_page(page_num, content) {
   // $('html, body').animate({ scrollTop: $(content).offset().top - 200 }, 'slow');
    ...
    var show_per_page = parseInt($(content).parent().find('.show_per_page1').val());
    start_from = page_num * show_per_page;
    end_on = start_from + show_per_page;
    $(content).children('.listData1').css('display', 'none').slice(start_from, end_on).css('display', 'block');
    $(content).parent().find('.current_page1').val(page_num);
